...
min_years, min_engine_volume, max_price = search_criteria

car_search = [
    (name, details) for name, details in car_data.items()
    if details[1] >= min_years and details[2] >= min_engine_volume and details[4] <= max_price]

found_cars = sorted(car_search, key=lambda x: x[1][4])
for car in found_cars[:5]:
    print(f"{car[0]}: ({car[1][0]}, {car[1][1]}, {car[1][2]}, {car[1][3]}, {car[1][4]})")
